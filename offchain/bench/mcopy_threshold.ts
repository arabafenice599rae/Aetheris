...
function main() {
  ...
  // if most payloads are small, MCOPY doesn't help. If median large, consider MCOPY in macro-runner kernels.
  const recommend = (p50 >= 512 || p99 >= 2048)
    ? "Consider MCOPY path (macro runners / heavy memcopy)"
    : "Stick to CALLDATACOPY / minimal mem usage";
