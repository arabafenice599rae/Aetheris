object "Kernel1153" {
  ...
  object "Runtime" {
    code {
      ...
      function callUnlock() {
        // Header must be >= 72B (deadline+bindTag, auth, hint64)
        if lt(calldatasize(), 72) { fail() }

        let w0 := calldataload(0)
        let w1 := calldataload(32)

        // word0: deadline (u64 LE in low 8B), bindTag (next 8B)
        ...
        // hint64 is stored at offset 64..71 (LE in your packer)
        // we store full word but compare full word for simplicity
        let hint := calldataload(64)
      ...
      function handleCallback() {
        // canonical bytes encoding: offset == 0x20
      ...
      // PackedOps inside callback data:
      // header 72B:
      //  word0: deadline|bindTag
      //  word1: auth (low 20B)
      //  word2: hint64 (stored in low 8B by packer; we compare whole word)
      //
      // ops stream begins at +72
      ...
      function run(ptr, end) {
        ...
        // compare full word (safe because we store full word)
        ...
        for { } lt(p, end) { } {
          ...
          case 0x02 { // TAKE (discipline mirror++)
            if iszero(call(gas(), PM, 0, payload, len, 0, 0)) { fail() }
            tstore(TS_MIRR, add(tload(TS_MIRR), 1))
          }
          case 0x03 { // CLEAR (discipline mirror--)
            if iszero(call(gas(), PM, 0, payload, len, 0, 0)) { fail() }
            let m := tload(TS_MIRR)
            if iszero(m) { fail() }
            tstore(TS_MIRR, sub(m, 1))
          }
          case 0x04 { // SETTLE (PM call)
            if iszero(call(gas(), PM, 0, payload, len, 0, 0)) { fail() }
          }
          case 0x05 { // CALLV: payload = value(32) || calldata
            if lt(len, 32) { fail() }
          ...
          case 0x06 { // ERC20XFER: payload = token(20) || amount(32); calls token.transfer(PM, amount)
            // Expect len == 52
            if iszero(eq(len, 52)) { fail() }

            // token address is left-aligned in calldata word; we want the 20 bytes
            ...
            mstore(add(m, 4), shl(96, PM))      // address to (left padded)
            mstore(add(m, 36), amt)
